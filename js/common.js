...
// ヘッダー注入(inject)がDOMContentLoaded後に実行されるページでは、
// #afterVideo がまだ存在しないタイミングで setupAfterVideo が呼ばれることがあるため、
// 常に「ヘッダー注入が完了しているはずのタイミング」で呼ぶようにするラッパー
function runSetupAfterVideo() {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', setupAfterVideo, { once: true });
  } else {
    setupAfterVideo();
  }
}
  ...
  if (hasPlayedIntro) {
    ...
    const loaderObserver = new MutationObserver(() => {
      if (loader.classList.contains('loaded')) {
        ...
        setTimeout(runSetupAfterVideo, 200); // 動画を出すタイミング
...
// スクロール連動で背景動画を再生/停止
document.querySelectorAll('.band .bg').forEach(video => {
  const observer = new IntersectionObserver((entries) => {
    ...
    threshold: 0.7 // 25%くらい見えたら再生開始(お好みで調整)
  });
